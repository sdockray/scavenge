...
function whatsHere (o, conf) {
  if (conf.find) {
    // o = o.then(context => console.log("opts: ", context))
    o = o.find(conf.find)
  }
  ...
  if (conf.filter) {
    o = o.then(function (context, data, next, done) {
      const filter = _.keys(conf.filter).every(
        key => data[key] && data[key].match(new RegExp(conf.filter[key]))
      )
      if (filter) {
        next(context, data)
      } else {
...
var executeMethod = (plugins, name, event) => previousData => {
  ...
  if (_.isArray(plugin.options)) {
    return plugin.options.reduce(
      (p, o) => p.then(d => action(d, o)),
...
function scrape (instructions, plugins) {
  const queue = new PromiseQueue(
    null,
    instructions.concurrency || defaults.concurrency
  )
  ...
  var o = osmosis.get(instructions.origin, undefined)
  o = whatsHere(o, instructions)
  o.then((context, data, next, done) => {
    queue.add(() =>
      execute(_.clone(data), plugins, 'onData').then(() => {
        done()
      })
    )
  })
    .done(() => {
      ...
      execute(null, plugins, 'onEnd').then(() => {
        console.log('DONE!')
      })
    })
    .error(e => console.log('e', typeof e))
}
